...
function resolveVotes(roomCode: string) {
    ...
    let reportMsg = 'لم يتم طرد أحد بسبب التخطي أو التعادل!';
    if (!isTie && ejectedId !== 'skip') {
        ...
        if (victim) {
            ...
            reportMsg = `تم قذف ${victim.name} في الفضاء!`;
        }
    }
    ...
    if (aliveImpostors === 0) {
        ...
        io.to(roomCode).emit(SocketEvents.SERVER_GAME_OVER, { winner: 'CREWMATES', text: reportMsg + ' فاز الأبرياء! 🎉' });
        return;
    } else if (aliveImpostors >= aliveCrewmates) {
        room.gameState = GameState.ENDED;
        io.to(roomCode).emit(SocketEvents.SERVER_GAME_OVER, { winner: 'IMPOSTORS', text: reportMsg + ' فاز القتلة! 😈' });
        return;
    }
...
io.on('connection', (socket) => {
    ...
    // ✅ إضافة الـ handler المفقود CLIENT_JOIN_RANDOM
    socket.on(SocketEvents.CLIENT_JOIN_RANDOM, (payload: { playerName: string }) => {
        try {
            const room = roomManager.joinRandom(socket.id, payload.playerName);
            socket.join(room.roomCode);
            socket.emit(SocketEvents.SERVER_JOIN_SUCCESS, { roomCode: room.roomCode, playerId: socket.id });
            gameEngine.broadcastRoomUpdate(room.roomCode);
        } catch (e: any) { socket.emit(SocketEvents.SERVER_ERROR, e.message); }
    ...
    socket.on(SocketEvents.CLIENT_UPDATE_SETTINGS, (payload: { roomCode: string; settings: any }) => {
    ...
    socket.on(SocketEvents.CLIENT_START_GAME, (payload: { roomCode: string }) => {
        ...
        if (room && room.hostId === socket.id && room.gameState === GameState.LOBBY) {
            ...
            setTimeout(() => {
                const current = roomManager.getRoom(payload.roomCode);
                if (current && current.gameState === GameState.STARTING) {
                    current.gameState = GameState.PLAYING;
                    gameEngine.broadcastRoomUpdate(current.roomCode);
                }
            }, 4000);
        }
    });
    ...
    socket.on(SocketEvents.CLIENT_REPORT_BODY, (payload: { roomCode: string }) => {
        ...
        if (room && room.gameState === GameState.PLAYING) {
            ...
            if (reporter && reporter.isAlive) {
                ...
                io.to(room.roomCode).emit(SocketEvents.SERVER_CHAT_RECEIVE, {
                    senderName: '🚨 بلاغ', senderColor: '#ff3333',
                    text: `قام اللاعب [${reporter.name}] بالتبليغ عن جثة! ابدأوا التصويت.`
                });
                ...
                const timerInterval = setInterval(() => {
                    ...
                    if (r && r.gameState === GameState.MEETING && r.meetingTimer !== undefined) {
                        ...
                        if (r.meetingTimer <= 0) {
                            ...
                            io.to(r.roomCode).emit(SocketEvents.SERVER_TICK_UPDATE, {
                                players: r.players, gameState: r.gameState, meetingTimer: r.meetingTimer
                            });
                        }
    ...
    socket.on(SocketEvents.CLIENT_SEND_CHAT, (payload: { roomCode: string; text: string }) => {
        ...
        if (room && room.gameState === GameState.MEETING) {
            ...
            if (sender) {
                // الأشباح لا يمكنهم الدردشة مع الأحياء
                if (!sender.isAlive) return;
                io.to(room.roomCode).emit(SocketEvents.SERVER_CHAT_RECEIVE, {
                    senderName: sender.name, senderColor: sender.color, text: payload.text
                });
            }
        }
    });
            ...
            if (voter && voter.isAlive && !voter.hasVoted) {
                ...
                if (totalVoted === alivePlayers.length) {
                    ...
                    io.to(room.roomCode).emit(SocketEvents.SERVER_TICK_UPDATE, {
                        players: room.players, gameState: room.gameState, meetingTimer: room.meetingTimer
                    });
                }
            }
        }
    ...
    socket.on(SocketEvents.CLIENT_KILL, (payload: { roomCode: string; targetId: string }) => {
        ...
        if (room && room.gameState === GameState.PLAYING) {
            ...
            if (killer && victim && killer.role === PlayerRole.IMPOSTOR && killer.isAlive && victim.isAlive) {
                ...
                if (now - (killer.lastKillTime || 0) < cooldownMs) return;

                const dist = Math.hypot(killer.position.x - victim.position.x, killer.position.y - victim.position.y);
                if (dist > 180) return;
                ...
                // فحص انتهاء اللعبة بعد القتل
                const aliveImpostors = room.players.filter(p => p.isAlive && p.role === PlayerRole.IMPOSTOR).length;
                const aliveCrewmates = room.players.filter(p => p.isAlive && p.role === PlayerRole.CREWMATE).length;

                if (aliveImpostors >= aliveCrewmates) {
                    room.gameState = GameState.ENDED;
                    io.to(room.roomCode).emit(SocketEvents.SERVER_GAME_OVER, {
                        winner: 'IMPOSTORS', text: 'فاز القتلة! السفينة تحت سيطرتهم الآن! 😈'
                    });
                    return;
                }
    ...
    socket.on(SocketEvents.CLIENT_MOVE, (payload: { roomCode: string; vec: { x: number; y: number } }) => {
        ...
        if (room && room.gameState === GameState.PLAYING) {
            ...
            if (player && player.isAlive) {
                ...
                else if (payload.vec.x < 0) player.direction = 'left';

                player.position.x = Math.max(GAME_CONSTANTS.PLAYER_RADIUS, Math.min(player.position.x, GAME_CONSTANTS.MAP_WIDTH - GAME_CONSTANTS.PLAYER_RADIUS));
                player.position.y = Math.max(GAME_CONSTANTS.PLAYER_RADIUS, Math.min(player.position.y, GAME_CONSTANTS.MAP_HEIGHT - GAME_CONSTANTS.PLAYER_RADIUS));
    ...
    socket.on('disconnect', () => {
        ...
        if (roomCode && room) {
            if (room.gameState === GameState.MEETING) {
                const alivePlayers = room.players.filter(p => p.isAlive);
                const totalVoted = alivePlayers.filter(p => p.hasVoted).length;
                if (alivePlayers.length > 0 && totalVoted === alivePlayers.length) resolveVotes(roomCode);
            }
            if (room.players.length > 0) gameEngine.broadcastRoomUpdate(roomCode);
        }
    });
});

// توجيه أي مسار للـ index.html
app.get('*', (_req, res) => {
    res.sendFile(path.join(clientDistPath, 'index.html'));
});

httpServer.listen(3000, () => console.log('🚀 السيرفر يعمل على البورت 3000'));
